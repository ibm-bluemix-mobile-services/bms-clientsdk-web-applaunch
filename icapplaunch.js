...
window.IBMAppLaunch = (function () {
    ...
    __ICUtils = function () {
        ...
        this.__clearUserContext = function () {
            ...
            IC.DAO.removeItem("CACHE_EXPIRATION");
        }
        ...
        this.__getRegistrationData = function () {
            ...
            registrationData.os = IC.Utils.__getOSName();
        ...
        this.__getUpdateRegistrationData = function () {
            ...
            delete registrationData.deviceId;
            return registrationData;
        }
        ...
        this.__displayMessage = function (title, subTitle, imageUrl, leftBtnJson, rightBtnJson) {
            d = document;
            var rightBtnMetricsCodes = [];
            var leftBtnMetricsCodes = [];
            if (d.getElementById("appLaunch-modalContainer")) {
                return;
            }
            ...
            var rightBtn = d.createElement('a');
            ...
            leftBtn.href = "javascript:document.getElementsByTagName(\"body\")[0].removeChild(document.getElementById(\"appLaunch-modalContainer\"));IBMAppLaunch.sendMetrics(" + leftBtnMetricsCodes + ");";
            rightBtn.href = "javascript:document.getElementsByTagName(\"body\")[0].removeChild(document.getElementById(\"appLaunch-modalContainer\"));IBMAppLaunch.sendMetrics(" + rightBtnMetricsCodes + ");";
            button_container.appendChild(leftBtn);
            button_container.appendChild(rightBtn);
    ...
    __URLBuilder = function (appLaunchRegion, appLaunchAppID, appLaunchDeviceID) {
        this.baseURL = 'https://applaunch' + appLaunchRegion + '/applaunch/v1/apps/'
    ...
    __URLBuilder.prototype = {
        __getAppRegistrationURL: function () {
            return this.baseURL + this.appID + '/devices'
        },
    ...
    function __destroy() {
        ...
        if (IC.Config.isInitialized) {
            var method = 'DELETE';
            var requestURL = IC.Config.__getURLBuilder().__getUserURL();
            var request = new __ICRESTInvoker(requestURL, method);
            request.addHeader('clientSecret', IC.Config.config.__getClientSecret())
            request.setCallBack(function (response) {
                if (response.status === 202) {
                    IC.Config.isInitialized = false;
                    IC.Utils.__clearUserContext();
                    dfd.resolve(IC.Utils.__generateSuccessResponse({}));
                } else {
                    dfd.reject(IC.Utils.__generateFailureResponse(errorCode.UNREGISTRATION_FAILURE, response.response));
                }
            });
            request.execute();
        }
    ...
    function __isFeatureEnabled(featureCode) {
        if (IC.Config.isInitialized) {
            ...
            for (var i = 0; i < ICAppLaunchFeatures.length; i += 1) {
    ...
    function __displayInAppMessages() {
        ...
        if (ICAppLaunchInAppMessages.length !== 0) {
            var message = ICAppLaunchInAppMessages[0];
            IC.Utils.__displayMessage(message.title, message.subtitle, message.imageUrl, message.buttons[0], message.buttons[1]);
        }
    }
